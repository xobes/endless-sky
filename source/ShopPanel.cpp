...
ShopPanel::TransactionResult ShopPanel::CanMoveToCargo() const
{
	return false;
}
...
ShopPanel::TransactionResult ShopPanel::CanDoTransaction(const SDL_Keycode key) const
{
	switch(key)
	{
		...
		case 's': // _Sell
			return CanSell();
		case 'r': // Sell Ship and put Outfits in Storage / Move to Storage
			if(isOutfitter)
		...
		case 'c': // to _Cargo
			if(isOutfitter)
			{
				...
				if(CanMoveToCargo())
				{
					...
					BuyToCargo(); // modifier applied within
			}
			break;
		case 'u': // _Uninstall
			if(isOutfitter)
